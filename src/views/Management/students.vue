...
<template>
  <div id="">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/home/students' }"
    ...
    <el-card>
        ...
        <el-col :span="10">
          ...
          >
            <el-select v-model="queryInfo.querytype" slot="prepend">
              ...
              <el-option label="学号" value="STID"></el-option>
      ...
      <el-table :data="userlist" border height="60vh">
        <el-table-column prop="currentid" align="center" width="50px"> </el-table-column>
    ...
    >
      ...
      >
        <el-form-item label="学号">
          <el-input v-model="editInfoForm.STID" disabled></el-input>
        </el-form-item>
        <el-form-item label="姓名">
          <el-input v-model="editInfoForm.name" disabled></el-input>
        </el-form-item>
        <el-form-item label="班级" >
          <el-select v-model="editInfoForm.classno" placeholder="请选择班级">
            <el-option
    ...
    >
      ...
      >
        <el-form-item label="学号" prop="STID">
          <el-input v-model.number="addUserForm.STID"></el-input>
        </el-form-item>
        ...
        <el-form-item label="班级" prop="classno">
          <el-select v-model="addUserForm.classno" placeholder="请选择">
            <el-option
              v-for="item in classlist"
              :key="item"
              :label="item"
    ...
    >
      <el-row>
       ...
       <el-alert
        v-if="!isrepeat"
        title="导入的数据必须是正确合法的才能进行批量添加(学号唯一、一个身份证号对应一位学生)"
        type="info"
        show-icon
        :closable="false"
      >
...
export default {
  ...
  data() {
    ...
    var checklen = (rule, value, callback) => {
      if (value) {
        if ((value + "").length !== 13) {
          callback(new Error("学号长度必须为13个数字"));
        } else {
          callback();
        }
      }
  ...
  methods: {
    getclasses(){
       SEARCH("", "queryall", "class").then((res) => {
        let classes = res.data.data;
        classes.forEach((item) => {
          this.classlist.push(item.classno);
        });
      });
    },
    keyDown(e) {
    ...
    delstudent() {
      DEL(this.clickusername, "student").then((res) => {
        let msg = res.data.status;
        if (msg == "success") {
          this.$message({
            message: "删除成功",
    ...
    editInfo(username) {
       GETINFO(username, "student").then((res) => {
     ...
     edituserinfo() {
      this.$refs.editFormRef.validate((valid) => {
        if (!valid) return;
        SETINFO(
          this.editInfoForm.STID,
          this.editInfoForm.classno,
          "student"
        ).then((res) => {
          let msg = res.data.status;
          if (msg == "success") {
 ...
 this.$refs.addFormRef.validate((valid) => {
    ...
    addstudentbyfile() {
      ...
      for (let [index, elem] of new Map(
        ...
        if (
          this.tablerule.STID.test(elem.STID) &&
          this.tablerule.classno.test(elem.classno) &&
          this.tablerule.cardno.test(elem.cardno) &&
          elem.name.length >= 2 &&
          elem.name.length <= 10
        ) {
          if(that.classlist.indexOf(elem.classno)==-1){
            flag = false;
          this.$message.error("第" + (index + 1) + "条记录班级号不存在");
          break;
          }
          for (let item1 of that.datalist) {
            if (elem.STID == item1.STID) {
              flag = false;
              break;
            }
          }
          if (!flag) {
            this.$message.error("第" + (index + 1) + "条记录学号已存在");
            break;
          }
          for (let item1 of that.datalist) {
      ...
      if (flag) {
        ADDBYFILE(this.xlscList, "student", true).then((res) => {
          let msg = res.data.status;
           if (msg == "success") {
              this.$message({
                message: "批量添加成功",
        ...
        if (tab && tab.length > 0) {
          tab[0].sheet.forEach((item) => {
            ...
            self.xlscList.forEach((item) => {
              if (item.STID === obj.STID || item.cardno === obj.cardno) {
                this.isrepeat = true;
              }
            });
